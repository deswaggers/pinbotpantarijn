...
class Easyrider(game.Mode):
        def __init__(self, game, priority):
                super(Easyrider, self).__init__(game, priority)
                self.game.sound.register_sound('bumper', sound_path+"bumpers.aiff")
                self.game.sound.register_sound('er_shootrightramp', speech_path+"er_shootrightramp.ogg")
                self.game.sound.register_sound('siren', sound_path+"siren1.ogg")
                self.game.sound.register_sound('easy_rider_jp', sound_path+"easy_rider_jp.ogg")

                self.game.sound.register_sound('er_shot_made', speech_path+"er_yeahbaby.ogg")
                self.game.sound.register_sound('er_shot_made', speech_path+"er_justridin.ogg")
                self.game.sound.register_sound('er_shot_made', speech_path+"er_meandmybike.ogg")
                self.game.sound.register_sound('er_shot_made', speech_path+"er_goingeasy.ogg")

                self.game.sound.register_sound('thatseasy', speech_path+"er_thatseasy.ogg")
                self.game.sound.register_sound('er_shootlitshots', speech_path+"er_shootallitshots.ogg")

                self.game.sound.register_music('easyrider_theme', music_path+"easyrider.ogg")

                self.game.lampctrl.register_show('shotmade', lampshow_path+"er_shotmade.lampshow")
...
## mode functions
        ...
        def shot_made(self):
             self.game.sound.play('er_shot_made')
        ...
        def update_status(self):
                if self.check_shots_made() == 1:
                        ...
                        self.knock_times(6)
                        self.shot_made()
                elif self.check_shots_made() == 7 and self.phase=='phase1':
                        self.shot_made()
                        self.game.base_game_mode.rampmove.move_ramp('down')
                        self.phase='phase2'
        ...
        def shootrampsound(self):
                self.game.sound.play('er_shootrightramp')

        def shootlitshotssound(self):
                self.game.sound.play('er_shootlitshots')
...
## switches
        ...
        def sw_outhole_active(self,sw):
                self.delay(name='stopeasyrider', event_type=None, delay=1.0, handler=self.stop_easyrider)
                self.game.coils.flipperEnable.enable()
                self.game.effects.gi_on()
